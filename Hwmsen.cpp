	  ...
	  mdata_fd(-1)
{
    ...
    mPendingEvents[step].sensor = ID_STEP_COUNTER;
    mPendingEvents[step].type = SENSOR_TTYPE_STEP_COUNTER;
	mPendingEvents[step].data[3] = SENSOR_STATUS_ACCURACY_HIGH;

	mPendingEvents[HUMIDITY].version = sizeof(sensors_event_t);
    mPendingEvents[HUMIDITY].sensor = ID_RELATIVE_HUMIDITY;
    mPendingEvents[HUMIDITY].type = SENSOR_TTYPE_RELATIVE_HUMIDITY;
	mPendingEvents[HUMIDITY].data[3] = SENSOR_STATUS_ACCURACY_HIGH;
	
	mPendingEvents[temperature].version = sizeof(sensors_event_t);
    mPendingEvents[temperature].sensor = ID_TEMPRERATURE;
    mPendingEvents[temperature].type = SENSOR_TYPE_TEMPERATURE;
	mPendingEvents[temperature].data[3] = SENSOR_STATUS_ACCURACY_HIGH;
...
int Hwmsen::FindDataFd() {
	...
	{
		ALOGE("sensor open hwmsensornum err");
		return fd;
	}
...
int Hwmsen::enable(int32_t handle, int en) {
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		...
		handle != ID_PROXIMITY && handle != ID_LIGHT&& handle != ID_STEP_COUNTER&& handle != ID_TEMPRERATURE&& handle != ID_RELATIVE_HUMIDITY)
	{
...
void Hwmsen::processEvent(int code, int value)
{
	...
	for(i =0; i < MAX_ANDROID_SENSOR_NUM; i++)
	{
		...
		case ID_STEP_COUNTER:
			...
			//int stepcounter =sensors_data.data[ID_STEP_COUNTER].values[0];
			//ALOGE("get sensor value 111[stepcounter] ===%d  \r\n",stepcounter);
			 mPendingMask |= 1<<step;
			 mPendingEvents[step].type = SENSOR_TTYPE_STEP_COUNTER;
			 mPendingEvents[step].sensor = sensors_data.data[ID_STEP_COUNTER].sensor;
			 //mPendingEvents[step].data[0]= (float)sensors_data.data[ID_STEP_COUNTER].values[0];
			// mPendingEvents[step].data[0]/=sensors_data.data[ID_STEP_COUNTER].value_divide;
			 mPendingEvents[step].u64.step_counter=(float)sensors_data.data[ID_STEP_COUNTER].values[0];
			 mPendingEvents[step].timestamp = sensors_data.data[ID_STEP_COUNTER].time;
			 ALOGE("get sensor value  3333[ID_STEP_COUNTER]====%d,%d \r\n",mPendingEvents[step].u64.step_counter,sensors_data.data[ID_STEP_COUNTER].values[0]);
			break;


	  case ID_RELATIVE_HUMIDITY:
		
			mPendingMask |= 1<<HUMIDITY;
			 mPendingEvents[HUMIDITY].type = SENSOR_TTYPE_RELATIVE_HUMIDITY;
			 mPendingEvents[HUMIDITY].sensor = sensors_data.data[ID_RELATIVE_HUMIDITY].sensor;
			// mPendingEvents[HUMIDITY].light = (float)sensors_data.data[ID_RELATIVE_HUMIDITY].values[0];
		    mPendingEvents[HUMIDITY].data[0]= (float)sensors_data.data[ID_RELATIVE_HUMIDITY].values[0];
			// mPendingEvents[step].data[0]/=sensors_data.data[ID_STEP_COUNTER].value_divide;
			 mPendingEvents[HUMIDITY].timestamp = sensors_data.data[ID_RELATIVE_HUMIDITY].time;
			 ALOGE("get hp06 hu value  3333[ID_STEP_COUNTER]====%d,%d \r\n", mPendingEvents[HUMIDITY].data[0],sensors_data.data[ID_RELATIVE_HUMIDITY].values[0]);
			break;
	  ...
	  case ID_TEMPRERATURE:
			...
			mPendingMask |= 1<<temperature;
			 mPendingEvents[temperature].type = SENSOR_TYPE_TEMPERATURE;
			 mPendingEvents[temperature].sensor = sensors_data.data[ID_TEMPRERATURE].sensor;
			// mPendingEvents[HUMIDITY].light = (float)sensors_data.data[ID_RELATIVE_HUMIDITY].values[0];
		    mPendingEvents[temperature].data[0]= (float)sensors_data.data[ID_TEMPRERATURE].values[0];
			// mPendingEvents[step].data[0]/=sensors_data.data[ID_STEP_COUNTER].value_divide;
			 mPendingEvents[temperature].timestamp = sensors_data.data[ID_TEMPRERATURE].time;
			 ALOGE("get hp06 tem value  3333[ID_STEP_COUNTER]====%d,%d \r\n", mPendingEvents[temperature].data[0],sensors_data.data[ID_TEMPRERATURE].values[0]);
			break;
...
int Hwmsen::setDelay(int32_t handle, int64_t ns)
{
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		handle != ID_ORIENTATION && handle != ID_GYROSCOPE && 
		handle != ID_LIGHT&& handle != ID_STEP_COUNTER&& handle != ID_TEMPRERATURE&& handle != ID_RELATIVE_HUMIDITY)
	{
